...
function FivePaisaClient(conf) {
  ...
  const MARKET_DEPTH_TOKEN_ROUTE='https://openapi.5paisa.com/marketfeed-token/token';
  var CLIENT_CODE = "";
  ...
  request_instance.defaults.headers.common["Authorization"] = "Bearer " + accessToken;
...
try{
  ...
  this.getMarketDepthBySymbol = function (DepthList) {
    ...
    var promise = new Promise(function (resolve, reject) {
      request_instance.post(MARKET_DEPTH_BY_SYMBOL_ROUTE, payload).then(response => {
        if (response.data.body.Data.length === 0) {
          reject({ err: "No info found" });
        } else {
          resolve(response.data.body);
   ...
   var req_data={
    ...
    "JWTToken":jwttoken
      ...
      if (response.data.Message=== "Success") {
        var setHeader={'Ocp-Apim-Subscription-Key': ocpKey,
...
this.getMarketStatus = function () {
  ...
  var promise = new Promise(function (resolve, reject) {
    request_instance.post(MARKET_STATUS_ROUTE, payload,{
      headers: headers
    }).then(response => {
      if (response.data.body.Data.length === 0) {
...
this.getTradeHistory = function (exchangeId) {
  ...
  var promise = new Promise(function (resolve, reject) {
    request_instance.post(TRADE_HISTORY_ROUTE, payload,{
      headers: headers
    }).then(response => {
...
this.createBasket =function(basketName){
  try{
    ...
    var promise = new Promise(function (resolve, reject) {
      ...
      request_instance.post(CREATE_BASKET_ROUTE, payload,{
        headers: headers
      }).then(response => {
        if (response.data.body.Status != 0) {
          reject(response.data.body.Message);
...
this.getOrderInBasket =function(basketID){
  try{
    ...
    var promise = new Promise(function (resolve, reject) {
      ...
      request_instance.post(GET_ORDERIN_BASKET_ROUTE, payload).then(response => {
        if (response.data.body.Status != 0) {
          reject(response.data.body.Message);
        } else {
          resolve(response.data.body);
...
this.addOrderInBasket =function(exch,exchtype,price,orderType,qty,scripCode,delvIntra,basketIDs,params){
  try{
    ...
    this.genericPayload.body.AtMarket=   params.AtMarket || basketOrderParams.AtMarket;
    this.genericPayload.body.StopLossPrice= params.StopLossPrice || basketOrderParams.StopLossPrice;
    this.genericPayload.body.IsStopLossOrder= params.IsStopLossOrder || basketOrderParams.IsStopLossOrder;
    this.genericPayload.body.IOCOrder= params.IOCOrder || basketOrderParams.IOCOrder;
    ...
    this.genericPayload.body.ValidTillDate= params.ValidTillDate || basketOrderParams.ValidTillDate;
    this.genericPayload.body.AHPlaced= params.AHPlaced || basketOrderParams.AHPlaced;
    this.genericPayload.body.PublicIP= params.PublicIP || basketOrderParams.PublicIP;
    this.genericPayload.body.DisQty= params.DisQty || basketOrderParams.DisQty;
    this.genericPayload.body.iOrderValidity= params.iOrderValidity || basketOrderParams.iOrderValidity;
...
this.positionConversion =function(exch,exchtype,tradeType,scripData,convertQty,convertFrom,convertTo){
  try{
    ...
    var promise = new Promise(function (resolve, reject) {
      ...
      request_instance.post(POSITION_CONVERSION_ROUTE, payload).then(response => {
        if (response.data.body.Status != 0) {
          reject(response.data.body.Message);
        } else {
          resolve(response.data.body);
...
this.getOptionChian =function(exch,symbol,expiryDate){
  try{
    ...
    var promise = new Promise(function (resolve, reject) {
      ...
      request_instance.post(GET_OPTION_CHAIN_ROUTE, payload).then(response => {
        if (response.data.body.Status != 0) {
          reject(response.data.body.Message);
        } else {
          resolve(response.data.body);
...
this.canceBulkOrder =function(exchOrderIDs){
  try{
    ...
    var promise = new Promise(function (resolve, reject) {
      ...
      request_instance.post(CANCEL_BULK_ORDER_ROUTE, payload).then(response => {
        if (response.data.body.Status != 0) {
          reject(response.data.body.Message);
        } else {
          resolve(response.data.body);
...
this.marketDepthToken= function(data,onmessagedisplay){
  try{
    const headers = {
     ...
     request.post({headers: headers, url: MARKET_DEPTH_TOKEN_ROUTE, method: 'POST'}, function (e, r, body) {
